...
export function Navbar({ auth }: { auth: any }) {
    ...
    return (
        <nav 
            ref={navRef} 
            className={cn(
                "fixed top-0 z-50 w-full border-b p-2 transition-all duration-300 backdrop-blur-md", // Reducido a p-2
            ...
            <div className="container mx-auto h-full flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-10">
                    ...
                    <div className="hidden lg:flex items-center gap-8">
                        {['Inicio', 'Tienda', 'Nosotros', 'Pedidos'].map((item) => (
                            <Link 
                                key={item} 
                ...
                <div className="flex items-center justify-end flex-1 gap-2">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="relative h-11 w-11">
                            ...
                            <span className="absolute top-1 right-1 h-5 w-5 text-xs font-bold flex items-center justify-center bg-primary text-primary-foreground rounded-full">0</span>
                        </Button>
                            ...
                            <Sheet>
                                ...
                                <SheetContent>
                                    <nav className="flex flex-col gap-6 mt-12">
                                        {['Inicio', 'Tienda', 'Nosotros', 'Pedidos'].map(item => (
                                            <Link key={item} href="/" className="text-2xl font-black border-b pb-2">{item}</Link>
                                        ))}
                                    </nav>
                                </SheetContent>
                            </Sheet>
